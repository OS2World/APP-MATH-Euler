...
static int XRotPaintAlignedString(Display *dpy, XFontStruct *font, float angle, Drawable drawable, GC gc, int x, int y, char *text, int align, int bg)
{
    ...
    xp=(short)((float)x-((float)item->cols_out/2 +hot_xp));
    yp=(short)((float)y-((float)item->rows_out/2 -hot_yp));
...
static RotatedTextItem *XRotCreateTextItem(Display *dpy, XFontStruct *font, float angle, char *text, int align)
{
	...
	item->cols_in=(short)((float)item->cols_in*style.magnify);
	item->rows_in=(short)((float)item->rows_in*style.magnify);
    }
	...
	for(i=((xl<0)?0:(int)xl); 
	    ...
	    /* rotate coordinates */
	    it=(short)((float)item->cols_in/2 + ( di*cos_angle + dj*sin_angle));
	    jt=(short)((float)item->rows_in/2 - (-di*sin_angle + dj*cos_angle));
...
static XImage *XRotMagnifyImage(Display *dpy, XImage *ximage)
{
    ...
    /* size of final image */
    cols_out=(short)((float)cols_in*style.magnify);
    rows_out=(short)((float)rows_in*style.magnify);
...
XPoint *XRotTextExtents(Display *dpy, XFontStruct *font, float angle, int x, int y, char *text, int align)
{
    ...
    xp_in[4].x=(short)(xp_in[0].x);
    xp_in[4].y=(short)(xp_in[0].y);
